...
/**
 * 首先简单了解下BitmapShader，BitmapShader是Shader的子类，Shader在三维软件中我们称之为着色器，
 * 所以通俗的理解，Shader的作用是给图像着色或者上色，BitmapShader允许我们载入一张图片来给图像着色,
 * 具体不做过多的解释，结尾贴出关于Shader的具体使用的文章
 * 所以其实根据上面对于BitmapShader的描述，其实就可以对圆角ImageView有一定的思路了吧，画一个圆角矩形，
 * 然后把本来画上去的图像着色到圆角矩形上，这样就实现了圆角的ImageView
 */
